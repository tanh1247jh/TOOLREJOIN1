...
class Utils:
    ...
    # ── Công cụ Ghi đè Cookie SQLite cho hệ thống Đăng nhập tự động ──
    @staticmethod
    def inject_cookie_to_db_file(db_path, cookie_value):
        """Ghi đè hoặc chèn giá trị bảo mật .ROBLOSECURITY trực tiếp vào cơ sở dữ liệu SQLite."""
        try:
            conn = sqlite3.connect(db_path)
            cur = conn.cursor()
            
            # Trích xuất chuỗi token thô của Cookie
            raw_token = cookie_value
            if ".ROBLOSECURITY=" in cookie_value:
                parts = cookie_value.split(".ROBLOSECURITY=")
                if len(parts) > 1:
                    raw_token = parts[1].split(";")[0].strip()
            
            # Kiểm tra xem bản ghi .ROBLOSECURITY đã tồn tại chưa
            cur.execute("SELECT COUNT(*) FROM cookies WHERE name='.ROBLOSECURITY'")
            exists = cur.fetchone()[0] > 0
            
            # Phân tích danh sách cột động của cấu trúc Chromium SQLite tương ứng trên thiết bị
            cur.execute("PRAGMA table_info(cookies)")
            cols = [col[1] for col in cur.fetchall()]
            
            # Tạo dấu mốc thời gian lớn (Microseconds Epoch) tương thích WebView
            now_epoch = int(time.time() * 1000000)
            expires_epoch = now_epoch + (10 * 365 * 24 * 60 * 60 * 1000000) # Thêm hạn dùng 10 năm
            
            if exists:
                cur.execute("UPDATE cookies SET value=? WHERE name='.ROBLOSECURITY'", (raw_token,))
                if "expires_utc" in cols:
                    cur.execute("UPDATE cookies SET expires_utc=? WHERE name='.ROBLOSECURITY'", (expires_epoch,))
            else:
                insert_cols = []
                insert_vals = []
                
                # Bản đồ ánh xạ giá trị mặc định cho cấu trúc cookie tiêu chuẩn
                mapping = {
                    "host_key": ".roblox.com",
                    "name": ".ROBLOSECURITY",
                    "value": raw_token,
                    "path": "/",
                    "expires_utc": expires_epoch,
                    "is_secure": 1,
                    "is_httponly": 1,
                    "has_expires": 1,
                    "is_persistent": 1,
                    ...
                    "source_scheme": 2,
                    "source_port": 443,
                    "creation_utc": now_epoch,
                    "last_access_utc": now_epoch
                }
                
                for col in cols:
                    if col in mapping:
                        insert_cols.append(col)
                        insert_vals.append(mapping[col])
                        
                if insert_cols:
                    q = f"INSERT INTO cookies ({', '.join(insert_cols)}) VALUES ({', '.join(['?']*len(insert_vals))})"
            ...
            logging.error(f"Lỗi ghi đè SQLite Cookie DB: {e}")
            return False

    @staticmethod
    def inject_cookie_adb(serial, package_name, cookie_value):
        """Sao chép cơ sở dữ liệu Cookies từ thiết bị giả lập qua ADB, sửa đổi rồi đồng bộ ngược trở lại."""
        adb_path = Utils.load_adb_path()
        sdcard_temp = f"/sdcard/cookies_inject_{int(time.time())}.db"
        local_temp = os.path.join(BASE_DIR, f"temp_inject_{serial.replace(':', '_')}.db")
        
        # Buộc tắt Roblox trước khi can thiệp cơ sở dữ liệu tránh tình trạng khóa tệp (SQLite locked)
        Utils.run_cmd(f'"{adb_path}" -s {serial} shell am force-stop {package_name}')
        time.sleep(1)

        # Chuyển tệp cơ sở dữ liệu SQLite của Roblox ra phân vùng dùng chung
        Utils.run_cmd(f'"{adb_path}" -s {serial} shell "su -c \'cp /data/data/{package_name}/app_webview/Default/Cookies {sdcard_temp}\'"')
        Utils.run_cmd(f'"{adb_path}" -s {serial} shell "su -c \'chmod 777 {sdcard_temp}\'"')
        
        # Tải tệp dữ liệu về máy chủ
        Utils.run_cmd(f'"{adb_path}" -s {serial} pull "{sdcard_temp}" "{local_temp}"')
        
        if not os.path.exists(local_temp):
            Utils.run_cmd(f'"{adb_path}" -s {serial} shell rm "{sdcard_temp}"')
            return False, "Không tìm thấy file Cookies cơ sở. Vui lòng mở ứng dụng Roblox ít nhất 1 lần để hệ thống tạo file."

        # Thực thi ghi đè Cookie
        success = Utils.inject_cookie_to_db_file(local_temp, cookie_value)
        if not success:
            try: os.remove(local_temp)
            except Exception: pass
            Utils.run_cmd(f'"{adb_path}" -s {serial} shell rm "{sdcard_temp}"')
            return False, "Ghi dữ liệu SQLite thất bại."

        # Đẩy dữ liệu đã sửa trở lại thiết bị giả lập
        Utils.run_cmd(f'"{adb_path}" -s {serial} push "{local_temp}" "{sdcard_temp}"')
        Utils.run_cmd(f'"{adb_path}" -s {serial} shell "su -c \'cp {sdcard_temp} /data/data/{package_name}/app_webview/Default/Cookies\'"')
        Utils.run_cmd(f'"{adb_path}" -s {serial} shell "su -c \'chmod 660 /data/data/{package_name}/app_webview/Default/Cookies\'"')
        Utils.run_cmd(f'"{adb_path}" -s {serial} shell "su -c \'chown $(stat -c %U /data/data/{package_name}) /data/data/{package_name}/app_webview/Default/Cookies\'"')
    ...
    @staticmethod
    def inject_cookie_local(package_name, cookie_value):
        """Sửa đổi tệp cơ sở dữ liệu cục bộ trực tiếp trên Android Root (Termux)."""
        if not IS_ANDROID:
            return False, "Hệ điều hành hiện tại không hỗ trợ Termux cục bộ."
            
        cookies_path = f"/data/data/{package_name}/app_webview/Default/Cookies"
        sdcard_path  = f"/sdcard/cookies_inject_temp_{int(time.time()*1000)}.db"
        
        # Tắt Roblox
        Utils.run_cmd(f"am force-stop {package_name}")
        time.sleep(1)
        ...
        try:
            try:
                ...
                Utils.run_cmd(f"su -c \"cp '{cookies_path}' '{sdcard_path}'\"")
                Utils.run_cmd(f"su -c \"chmod 777 '{sdcard_path}'\"")

            if not os.path.exists(sdcard_path):
                return False, "Không tìm thấy file Cookies cơ sở. Vui lòng mở ứng dụng Roblox ít nhất 1 lần để hệ thống tạo file."

            success = Utils.inject_cookie_to_db_file(sdcard_path, cookie_value)
            if not success:
                try: os.remove(sdcard_path)
                except Exception: pass
                return False, "Lỗi khi cập nhật cơ sở dữ liệu SQLite."

            # Sao chép ngược tệp cấu hình đã ghi đè
            try:
                shutil.copy2(sdcard_path, cookies_path)
            except Exception:
                Utils.run_cmd(f"su -c \"cp '{sdcard_path}' '{cookies_path}'\"")
                Utils.run_cmd(f"su -c \"chmod 660 '{cookies_path}'\"")
                Utils.run_cmd(f"su -c \"chown $(stat -c %U /data/data/{package_name}) '{cookies_path}'\"")
...
class AppCLI:
    ...
    def run(self):
        ...
        while True:
            ...
            print(f"   {C.SUC}[10]{C.TXT} login Cookie (pb beta) ")
            print(f"   {C.ERR}[11]{C.TXT} Đóng hệ thống & Thoát chương trình")
            print(f"  {C.GRAY}──────────────────────────────────────────────────────────────────────────{C.RESET}")
            
            choice = input(f"\n  {C.H2}👉 Nhập lựa chọn thực thi (1-11): {C.RESET}").strip()
            ...
            if opt == "1":
                ...
                try:
                    ...
                    if choice_idx == len(exec_names) + 1:
                        ...
                        custom_path = EXCULATORS[executor]
                except (ValueError, IndexError):
                    print(f"  {C.ERR}[-] Tùy chọn không hợp lệ.{C.RESET}")
                    time.sleep(1)
                    continue
    ...
    # ── Chức năng 10: Nhập và tự động Đăng nhập Cookie từ file TK|MK|Cookie ──────
    def auto_login_cookie_menu(self):
        Utils.clear_screen()
        print(f"  {C.H2}🔑 [ TỰ ĐỘNG IMPORT & LOGIN COOKIE TỪ FILE ]{C.RESET}")
        print(f"  {C.GRAY}────────────────────────────────────────────────────────────────────────{C.RESET}")
        print("  Định dạng dòng mẫu trong file: Tên_TK|Mật_Khẩu|Cookie_Roblox")
        print("  Hệ thống sẽ nạp danh sách tài khoản, hỗ trợ bạn chọn giả lập và tự động")
        print("  đăng nhập vào Roblox mà không cần thao tác tay thủ công trên giả lập.\n")
        
        file_path = input(f"  {C.H2}👉 Nhập đường dẫn file (Mặc định: accounts.txt): {C.RESET}").strip()
        if not file_path:
            file_path = "accounts.txt"
            
        if not os.path.exists(file_path):
            # Tạo file mẫu tự động giúp người dùng dễ tiếp cận định dạng cấu hình
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write("AccountUsername|Password123|_.ROBLOSECURITY_Cookie_Here\n")
                print(f"  {C.WRN}[!] File {file_path} không tồn tại. Hệ thống đã khởi tạo file mẫu.{C.RESET}")
                print("  Vui lòng điền thông tin tài khoản vào file rồi khởi động lại tính năng.")
            except Exception as e:
                print(f"  {C.ERR}[-] Không thể tạo file mẫu: {e}{C.RESET}")
            input("\n  Nhấn Enter để tiếp tục...")
            return

        # Đọc dữ liệu tài khoản từ tệp tin văn bản chỉ định
        accounts = []
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    # Tìm chuỗi cookie (thường nằm ở phần tử thứ 3 hoặc chứa ký tự đặc thù)
                    cookie_val = ""
                    for p in parts[2:]:
                        p_stripped = p.strip()
                        if "_|" in p_stripped or ".ROBLOSECURITY=" in p_stripped:
                            cookie_val = p_stripped
                            break
                    if not cookie_val:
                        cookie_val = parts[2].strip()
                        
                    if not cookie_val:
                        continue
                        
                    # Chuẩn hóa tiền tố Cookie nếu thiếu
                    if not cookie_val.startswith(".ROBLOSECURITY="):
                        if cookie_val.startswith("_|"):
                            cookie_val = f".ROBLOSECURITY={cookie_val}"
            ...
            print(f"  {C.ERR}[-] Không thể đọc tệp tin: {e}{C.RESET}")
            input("\n  Nhấn Enter để tiếp tục...")
            return

        if not accounts:
            print(f"  {C.ERR}[-] Không tìm thấy tài khoản hợp lệ nào trong file (Định dạng: TK|MK|Cookie).{C.RESET}")
            input("\n  Nhấn Enter để tiếp tục...")
            return

        print(f"  {C.SUC}[+] Phát hiện {len(accounts)} tài khoản trong file.{C.RESET}")
        time.sleep(1)

        # Truy xuất danh sách giả lập ADB hiện hành
        devices = Utils.get_adb_devices()
        
        for idx, acc in enumerate(accounts, 1):
            Utils.clear_screen()
            print(f"  {C.H2}⚙️ [ XỬ LÝ TÀI KHOẢN {idx}/{len(accounts)}: {C.SUC}{acc['username']}{C.H2} ]{C.RESET}")
            print(f"  {C.GRAY}────────────────────────────────────────────────────────────────────────{C.RESET}")
            print(f"  Tài khoản: {acc['username']}")
            print(f"  Cookie:    {acc['cookie'][:35]}...")
            print(f"  {C.GRAY}────────────────────────────────────────────────────────────────────────{C.RESET}")
            
            # Quét tìm mục tiêu cài đặt trên giả lập và cục bộ
            available_targets = []
            target_idx = 1
            
            if devices:
                print("  Chọn giả lập và gói Roblox đích để đăng nhập tự động:")
                for dev in devices:
                    pkgs = Utils.detect_roblox_packages_adb(dev)
                    if pkgs:
                        for pkg_name, pkg_info in pkgs.items():
                            print(f"   {C.CYAN}[{target_idx}]{C.TXT} Giả lập: {C.WHITE}{dev}{C.TXT} | Gói: {C.GREEN}{pkg_info['displayName']}{C.TXT} ({pkg_name})")
                            available_targets.append({
                                "mode": "adb",
                                "serial": dev,
                                "package": pkg_name,
                                "display": f"Giả lập: {dev} - {pkg_info['displayName']}"
                            })
                            target_idx += 1
            
            if IS_ANDROID:
                local_pkgs = Utils.detect_all_roblox_packages()
                if local_pkgs:
                    for pkg_name, pkg_info in local_pkgs.items():
                        print(f"   {C.CYAN}[{target_idx}]{C.TXT} Thiết bị cục bộ (Root) | Gói: {C.GREEN}{pkg_info['displayName']}{C.TXT}")
                        available_targets.append({
                            "mode": "local",
                            "serial": None,
                            "package": pkg_name,
                            "display": f"Nội bộ - {pkg_info['displayName']}"
                        })
                        target_idx += 1

            print(f"   {C.YELLOW}[S]{C.TXT} Bỏ qua tài khoản này (Skip)")
            print(f"   {C.RED}[Q]{C.TXT} Thoát tiến trình nạp (Quit)")
            print(f"  {C.GRAY}────────────────────────────────────────────────────────────────────────{C.RESET}")
            
            choice = input(f"  {C.H2}👉 Nhập lựa chọn: {C.RESET}").strip().lower()
            
            if choice == 'q':
                break
            elif choice == 's' or not choice:
                print(f"  [*] Đã bỏ qua tài khoản {acc['username']}.")
                time.sleep(0.5)
                continue
                
            try:
                sel_idx = int(choice) - 1
                if 0 <= sel_idx < len(available_targets):
                    target = available_targets[sel_idx]
                    
                    # Xác thực nhanh thông tin tài khoản qua API Roblox
                    print(f"  [*] Đang kiểm tra tính hiệu lực của Cookie...")
                    user_api = RobloxUser(cookie=acc["cookie"])
                    uid = user_api.fetch_authenticated_user()
                    if not uid:
                        print(f"  {C.ERR}[-] Cookie không hợp lệ hoặc đã hết hạn!{C.RESET}")
                        ans = input("  Vẫn tiếp tục đăng nhập đè vào hệ thống? (y/n): ").strip().lower()
                        if ans != 'y':
                            continue
                        uid = "000000"
                        username_actual = acc["username"]
                    else:
                        username_actual = user_api.username
                        print(f"  {C.SUC}[+] Xác thực thành công: {username_actual} (ID: {uid}){C.RESET}")

                    # Thực thi ghi đè Cookie bảo mật
                    if target["mode"] == "adb":
                        print(f"  [*] Đang ghi đè Cookie vào giả lập {target['serial']}...")
                        success, msg = Utils.inject_cookie_adb(target["serial"], target["package"], acc["cookie"])
                    else:
                        print(f"  [*] Đang ghi đè Cookie vào thiết bị nội bộ...")
                        success, msg = Utils.inject_cookie_local(target["package"], acc["cookie"])
                        
                    if success:
                        print(f"  {C.SUC}[+] Đăng nhập và nạp Cookie thành công!{C.RESET}")
                        
                        # Tự động đồng bộ và liên kết cấu hình nạp vào multi_configs.json để người dùng Rejoin tiện lợi
                        configs = Utils.load_multi_configs()
                        profile_key = f"{target['package']}_{target['serial'].replace('.', '_').replace(':', '_')}" if target['serial'] else target['package']
                        
                        existing_cfg = configs.get(profile_key, {})
                        configs[profile_key] = {
                            "username":    username_actual,
                            "userId":      uid,
                            "placeId":     existing_cfg.get("placeId", "16732694052"), # Mặc định Fisch
                            "gameName":    existing_cfg.get("gameName", "Fisch"),
                            "linkCode":    existing_cfg.get("linkCode", None),
                            "delaySec":    existing_cfg.get("delaySec", 30),
                        ...
                        Utils.save_multi_configs(configs)
                        print(f"  {C.SUC}[+] Đã đồng bộ tài khoản thành công vào hệ thống Rejoin tự động.{C.RESET}")
                        
                        # Tùy chọn khởi chạy ứng dụng Roblox ngay lập tức
                        launch_choice = input("  Khởi chạy Roblox ngay lập tức? (y/n): ").strip().lower()
                        if launch_choice == 'y':
                            Utils.launch(
                                place_id=configs[profile_key]["placeId"],
                                link_code=configs[profile_key].get("linkCode"),
                                package_name=target["package"],
                                mode=target["mode"],
                                adb_serial=target["serial"]
                            )
                    else:
                        print(f"  {C.ERR}[-] Không thể đăng nhập: {msg}{C.RESET}")
                        input("\n  Nhấn Enter để tiếp tục...")
                else:
                    print(f"  {C.ERR}[-] Lựa chọn nằm ngoài phạm vi hoạt động.{C.RESET}")
                    time.sleep(1)
            except Exception as ex:
                print(f"  {C.ERR}[-] Lỗi trong quá trình xử lý: {ex}{C.RESET}")
                input("\n  Nhấn Enter để tiếp tục...")
                
        print(f"\n  {C.SUC}[+] Hoàn tất quy trình xử lý danh sách tài khoản từ file.{C.RESET}")
